...
void main()
{
    ...
    total = omp_get_wtime();
    printf("%s", "Beginning Simulated Annealing!\n");
    #pragma omp parallel for schedule(static, SA_CHUNK_SIZE)
    ...
    output = fopen("./results/PSAResults.txt", "w+");
    if (!output)
    {
    ...
    {
        printf("%s", "File opened sucessfully\n");
    }
    /*
    printf("Loop %d:\nOrigin String: ", l + 1);
    for(i=0;i<STRING_LENGTH-1;i++)
