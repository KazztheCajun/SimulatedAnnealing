...
double ssa_rand_double(double min, double max) 
{
...
void ssa_new_string(double *c)
{
    int i;
    for (i = 0; i < STRING_LENGTH; i++)
    {
        c[i] = ssa_rand_double(-5.12, 5.12); // generate random value
...
void ssa_get_neighbors(double *n, double *c)
{
    int i, j, index = 0;
    for (i = 0; i < STRING_LENGTH; i++) // for each column in neighborhood
    {
        for (j = 0; j < STRING_LENGTH; j++) // for each row in neighbor string
        {
            ...
            {
                n[i*STRING_LENGTH + j] = ssa_rand_double(-5.12, 5.12);
            }
...
void ssa_copyString(double *this, double *that) // copies the info from this into that
{
    int i;
    for(i = 0; i < STRING_LENGTH; i++)
    {
...
int ssa_isSameString(double *a, double *b) // returns 1 if strings contain the same values, returns 0 if not
{
    int i = 0;
...
void ssa_simulated_annealing(double *min_strings, double *origin_strings, double *local_minima, double *timeData, int *loopNum, int loop, double STARTING_HEAT, int ANNEALING_STEPS, int ALPHA)
{
    ...
    while(temp > 1 || repeat < MAX_REPEATED_VALUE)
    {
        ...
        while(step < ANNEALING_STEPS)
        {
            ...
            ssa_get_neighbors(neighborhood, current);
            // save the last current value for comparison
            ssa_copyString(current, last);
            //printf("Current: ");
            //ssa_printString(current);
            ...
            //printf("Neighbor: ");
            //ssa_printString(neighbor);
            // calculate the value of the current (vc) and neighbor (vn)
            vc, vn = 0;
            vc = ssa_rastrigin(current);
            vn = ssa_rastrigin(neighbor);
        ...
        if (ssa_isSameString(current, last))
        {
            repeat++;
